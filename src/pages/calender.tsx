    ...
    return (
        <main className={`flex min-h-screen h-full flex-col items-center ${inter.className}`}>
            <div className="grid grid-cols-4 lg:grid-cols-6 w-full h-full place-items-center mb-20">
                {
                    days.map(day => (
                        <Link href={"/" + day} key={day} className={`
                        ${
                            ...
                            dayjs().month(11).date(toDay.month() === 11 && toDay.date() > 24 ? 24 : day).hour(0).minute(0).second(0).millisecond(0).isSameOrBefore(dayjs(toDay))
                                ?
                            ...
                            text-red-600 text-center w-14 h-14 lg:w-[7vw] lg:h-[7vw] flex flex-col justify-center border-l 
                        border-r border-y border-l-dashed bg-white bg-opacity-40 text-xl`}>
                            <p>{day}</p>
                        </Link>
                    )
                    )
